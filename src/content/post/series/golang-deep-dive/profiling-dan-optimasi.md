...
2. Bagian mana yang paling banyak mengalokasikan memori (*heap*).
3. Dimana terjadi hambatan pada goroutine (*contention*).

## Langkah 1: Mengambil Data Profil

Cara termudah untuk menggunakan pprof pada web server adalah dengan meng-import-nya secara *blank* (`_`):
...
3.  **Pre-allocate Slice**: Jika kamu tahu ukuran akhir sebuah slice, gunakan `make([]T, 0, capacity)` untuk menghindari proses *copy* memori saat slice tumbuh.
...
Namun ingat saran dari Donald Knuth: *"Premature optimization is the root of all evil."* Pastikan kodemu benar dan bersih dulu, baru lakukan optimasi jika data menunjukkan adanya masalah performa.
